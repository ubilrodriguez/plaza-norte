...
class Productos extends Controllers
{
	...
	public function Productos()
	{
		if (empty($_SESSION['permisosMod']['r'])) {
			header("Location:" . base_url() . '/dashboar');
		}
		$data['page_tag'] = "Productos";
		$data['page_title'] = "DETALLE DE PRODUCTOS <small>Tienda Virtual</small>";
	...
	public function getSelectNive3()
	{
		$htmlOptions = "";
		$arrData = $this->model->selectnivel3();
		if (count($arrData) > 0) {
			for ($i = 0; $i < count($arrData); $i++) {
				if ($arrData[$i]) {
					$htmlOptions .= '<option value="' . $arrData[$i]['idn3'] . '">' . $arrData[$i]['nomgru'] . '</option>';
				}
			}
		}
		echo $htmlOptions;
